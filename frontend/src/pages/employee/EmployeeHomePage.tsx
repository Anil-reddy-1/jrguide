    ...
    <div className="animate-fade-in space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <div className="mt-2 flex items-center gap-3 text-sm text-navy-300">
              <span>
                Onboarding {progress < 100 ? "In Progress" : "Complete"}
              </span>
            </div>
          </div>
      ...
      <div className="stagger-children grid gap-4 sm:grid-cols-3">
        <StatCard
          ...
          detail={`${completedTasks} completed, ${tasks.length - completedTasks} remaining`}
          icon={<CheckSquare size={18} />}
        />
        <StatCard
          label="Overall Progress"
          value={`${progress}%`}
          detail={`${completedTasks} of ${tasks.length} tasks done`}
        ...
        <StatCard
          ...
          detail={`${uploadedDocs} uploaded, ${docs.length - uploadedDocs} pending`}
          ...
          {todayTasks.length === 0 ? (
            ...
            <div className="space-y-3">
              {todayTasks.map((task: any) => (
                ...
                >
                  <div
                    className={`flex h-8 w-8 items-center justify-center rounded-full ${task.status === "in_progress" ? "bg-blue-50 text-blue-600" : "bg-slate-100 text-slate-400"}`}
                  >
                    <CheckSquare size={14} />
                  </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            {[
              ...
              <Link
                key={a.to}
                to={a.to}
                className="flex flex-col items-center gap-2 rounded-xl border border-slate-100 p-4 text-center transition-all hover:border-navy-200 hover:bg-navy-50/30 hover:shadow-sm"
              >
