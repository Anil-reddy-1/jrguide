...
export const HrFaqManagerPage = () => {
  ...
  const createMutation = useMutation({
    mutationFn: (data: any) =>
      apiClient("/api/faqs", { method: "POST", body: data }),
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: string; data: any }) =>
      apiClient(`/api/faqs/${id}`, { method: "PATCH", body: data }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["faqs"] });
      setShowModal(false);
      setEditFaq(null);
  ...
  const toggleMutation = useMutation({
    mutationFn: (id: string) =>
      apiClient(`/api/faqs/${id}/toggle`, { method: "PATCH" }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["faqs"] });
        ...
        {filtered.map((faq: any) => (
          ...
          >
            <div className="flex items-center gap-3">
              ...
              {faq.category && (
                <StatusBadge variant="info" label={faq.category} />
              )}
              <div className="flex items-center gap-1">
                <button
                  type="button"
                  onClick={() => toggleMutation.mutate(faq.id)}
